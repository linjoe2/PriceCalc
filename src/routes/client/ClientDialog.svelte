...
<div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4 sm:py-8">
    <div class="flex flex-col sm:flex-row sm:justify-between gap-2 sm:gap-0">
        <h1 class="text-2xl sm:text-3xl font-bold mb-3 sm:mb-6 text-gray-900">{clientData.name} {clientData.lastname}</h1>
        <h1 class="text-xl sm:text-3xl font-bold mb-3 sm:mb-6 text-gray-300">{clientData.subcontractors?.businessname}</h1>
    </div>
    <div class="bg-white rounded-lg shadow-lg p-4 sm:p-6 md:p-8">
        ...
        <div class="overflow-x-auto -mx-4 sm:mx-0">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Laatste update</th>
                        <th class="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Offertenaam</th>
                        <th class="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                        <th class="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Acties</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    {#each clientData?.projects ?? [] as project}
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap text-sm cursor-pointer" on:click={() => window.location.href = `/project/view/${project.$id}`}>
                            ...
                            <td class="px-2 sm:px-4 py-3 sm:py-4 whitespace-nowrap text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 sm:mt-8">
            <button
            ...
            >
                ...
                <span class="whitespace-nowrap">Klant bewerken</span>
            </a>
        </div>
    </div>
</div>